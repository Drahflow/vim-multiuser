    ...
    char_u	*fname;
{
    ...
    r = buf_write(buf, fname, NULL, (linenr_T)1, ml_line_count(buf),
    ...
    exarg_T	*eap;
{
    ...
    if (buf_write(curbuf, tmp_orig, NULL,
		(linenr_T)1, ml_line_count(curbuf),
    ...
    linenr_T	lnum;
{
    ...
    if (lnum < 1 || lnum > ml_line_count(buf) + 1)
	return 0;
    ...
    win_T	*towin;
{
	 ...
	 * filler lines. */
	towin->w_topline = ml_line_count(towin->w_buffer)
				       - (ml_line_count(frombuf) - lnum);
    }
    else
    {
    ...
    if (towin->w_topline > ml_line_count(towin->w_buffer))
    {
	towin->w_topline = ml_line_count(towin->w_buffer);
    ...
    exarg_T	*eap;
{
	...
	if (eap->cmdidx == CMD_diffget
		&& eap->line1 == ml_line_count(curbuf)
		...
		&& u_save(lnum - 1, lnum + count) != FAIL)
	{
		...
		buf_empty = ml_line_count(curbuf) == 1;
		...
		if (nr > ml_line_count(curtab->tp_diffbuf[idx_from]))
		    break;
		...
		if (p != NULL)
		{
		    ...
		    vim_free(p);
		    ++added;
		    if (buf_empty && ml_line_count(curbuf) == 2)
		    {
    ...
    linenr_T	lnum3;
{
	...
	if (dp->df_lnum[idx1] > lnum1)
	{
	    ...
	    /* don't end up past the end of the file */
	    if (lnum2 > ml_line_count(buf2))
		lnum2 = ml_line_count(buf2);
    ...
    win_T	*wp;
{
    ...
    if (dp == NULL)
	return ml_line_count(wp->w_buffer) - (ml_line_count(curbuf) - lnum);
