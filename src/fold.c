    ...
    linenr_T	bot;
{
     ...
     * of the buffer. */
    if (top > ml_line_count(wp->w_buffer))
	top = ml_line_count(wp->w_buffer);
    ...
    if (start > end && end < ml_line_count(wp->w_buffer))
	end = start;
    while (!got_int)
    {
	/* Always stop at the end of the file ("end" can be past the end of
	 * the file). */
	if (fline.lnum > ml_line_count(wp->w_buffer))
	    break;
	if (fline.lnum > end)
	{
    ...
    int		topflags;	/* flags used by containing fold */
{
    ...
    int		i;
    int		finish = FALSE;
    linenr_T	linecount = ml_line_count(flp->wp->w_buffer) - flp->off;
    int		concat;
    ...
    fline_T	*flp;
{
	...
	if (lnum == 1 || lnum == ml_line_count(buf))
	    flp->lvl = 0;
	else
	    flp->lvl = -1;
    }
    ...
    fline_T	*flp;
{
    ...
    flp->start = 0;
    if (lnum < ml_line_count(flp->wp->w_buffer))
    {
