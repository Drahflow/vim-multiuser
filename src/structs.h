...
/*
 * the buffer versions at various states of synchronisation
 * TODO: a more efficient implementation should be possible
 */
typedef struct remline_line
{
    int		len;
    int		local_lnum;	/* -1 if no local line exists (or is known), 1-based lnum otherwise */
    char_u	*data;
} remline_line_T;
...
{
    int			line_count;
    remline_line_T	*lines; 	/* zero-based array */
} remline_T;

/*
 * a remote memline is the connection to a memline server
 * for distributed editing
 */
typedef struct remote_memline
{
    int		ml_socket;		/* socket to remote memline server */
    int		ml_checkpoint_cur;	/* the checkpoint of the last user update */
    int		ml_checkpoint_sent;	/* the last checkpoint sent to the server */
    int		ml_checkpoint_recv;	/* the last checkpoint echo received from the server */
    remline_T	ml_incoming;		/* the version incoming master updates apply to */
    int		ml_incoming_version;	/* the version number of the incoming version */
    char_u	*in_buf;		/* the network read buffer */
    char_u	*in_buf_fill;           /* the next byte to read from the network */
} remote_memline_T;

/*
 * the abstract interface to a (possibly remote) memline
 */
typedef struct memline
{
    local_memline_T	ml_local;	/* local memline (might exist for redundancy purposes) */
    int			ml_has_remote;  /* whether the remote part is meaningful */
    remote_memline_T	ml_remote;	/* remote memline (if both exists, this is the primary) */
} memline_T;
