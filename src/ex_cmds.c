    ...
    linenr_T	dest;
{
     ...
     */
    last_line = ml_line_count(curbuf);
 ...
 */
    ...
    read_linecount = ml_line_count(curbuf);
	...
	read_linecount = ml_line_count(curbuf) - read_linecount;
    ...
    exarg_T	*eap;
{
	...
	if (alt_buf != NULL && ml_has_memfile(alt_buf))
	{
	 ...
	 */
	if (	   (eap->line1 != 1
		    || eap->line2 != ml_line_count(curbuf))
		&& !eap->forceit
		&& !eap->append
		&& !p_wa)
	{
...
# endif
	...
	if (buf == NULL)
	    goto theend;
	if (!ml_has_memfile(buf))		/* no memfile yet */
	{
	    oldbuf = FALSE;
	    buf->b_nwindows = 0;
	}
 ...
 */
	...
	if (p_ur < 0 || ml_line_count(curbuf) <= p_ur)
	{
	    ...
	    u_sync(FALSE);
	    if (u_savecommon(0, ml_line_count(curbuf) + 1, 0, TRUE)
								     == FAIL)
		goto theend;
	    u_unchanged(curbuf);
...
# endif
	...
	{
	    if (exmode_active)
		curwin->w_cursor.lnum = ml_line_count(curbuf);
	    beginline(BL_WHITE | BL_FIX);
	}
    }
    ...
    exarg_T	*eap;
{
    ...
    char_u	*p;
    int		vcol;
    int		empty = (ml_flags(curbuf) & ML_EMPTY);
    ...
    curbuf->b_op_start.lnum = (eap->line2 < ml_line_count(curbuf)) ?
	eap->line2 + 1 : ml_line_count(curbuf);
    if (eap->cmdidx != CMD_append)
    ...
    exarg_T	*eap;
{
    ...
    {
	if (ml_flags(curbuf) & ML_EMPTY)	    /* nothing to delete */
	    break;
	ml_delete(eap->line1, FALSE);
    }
    ...
    exarg_T	*eap;
{
    ...
    if (end > ml_line_count(curbuf))
	end = ml_line_count(curbuf);

    if (curs > ml_line_count(curbuf))
	curs = ml_line_count(curbuf);
    ...
    exarg_T	*eap;
{
    ...
    linenr_T	last_line= 0;		/* below last changed line AFTER the
					 * change */
    linenr_T	old_line_count = ml_line_count(curbuf);
    linenr_T	line2;
	...
	eap->line1 = eap->line2;
	eap->line2 += i - 1;
	if (eap->line2 > ml_line_count(curbuf))
	    eap->line2 = ml_line_count(curbuf);
    }
	...
	if (nmatch)
	{
		...
		if (do_ask)
		{
                    // FIXME: unlock buffer and try to continue search after re-lock
		    int typed = 0;
		...
		if (nmatch > ml_line_count(curbuf) - sub_firstlnum + 1)
		{
		    nmatch = ml_line_count(curbuf) - sub_firstlnum + 1;
		    skip_match = TRUE;
		}
...
skip:
	 ...
	 * deleted lines). */
	i = ml_line_count(curbuf) - old_line_count;
    ...
    char_u	*cmd;
{
    ...
    global_busy = 1;
    old_lcount = ml_line_count(curbuf);
	...
	msgmore(ml_line_count(curbuf) - old_lcount);
}
