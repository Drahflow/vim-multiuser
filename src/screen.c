...
# endif
	...
	{
	    wp->w_botline = ml_line_count(buf) + 1;
#ifdef FEAT_DIFF
...
# define VCOL_HLC (vcol)
	...
	if (lnum < ml_line_count(wp->w_buffer))
	{
...
# define this_ru_col ru_col
     ...
     */
    if (wp->w_cursor.lnum > ml_line_count(wp->w_buffer))
	return;
...
# endif
    ...
    if (       redraw_cmdline
	    ...
	    || ml_line_count(wp->w_buffer) != wp->w_ru_line_count
#ifdef FEAT_DIFF
...
# endif
	...
	wp->w_ru_line_count = ml_line_count(wp->w_buffer);
#ifdef FEAT_DIFF
    ...
    win_T	*wp;
{
	...
	lnum = ml_line_count(wp->w_buffer);
