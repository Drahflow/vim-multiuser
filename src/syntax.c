    ...
    linenr_T	lnum;
{
    ...
    if (VALID_STATE(&current_state)
	    && current_lnum < lnum
	    && current_lnum < ml_line_count(syn_buf))
    {
	...
	dist = ml_line_count(syn_buf) / (syn_block->b_sst_len - Rows) + 1;
...
syn_stack_alloc()
{
    ...
    len = ml_line_count(syn_buf) / SST_DIST + Rows * 2;
    ...
    int		extra;		/* extra chars for offset to start */
{
    ...
    if (result->lnum > ml_line_count(syn_buf))
	col = 0;
    else if (off != 0)
    {
    ...
    int		extra;	    /* extra chars for offset to end */
{
	...
	result->lnum = ml_line_count(syn_buf);
