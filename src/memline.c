    ...
    buf_T	*buf;
{
     ...
     */
    buf->b_ml.ml_local.ml_stack_size = 0; /* no stack yet */
    buf->b_ml.ml_local.ml_stack = NULL;	/* no stack yet */
    buf->b_ml.ml_local.ml_stack_top = 0;	/* nothing in the stack */
    buf->b_ml.ml_local.ml_locked = NULL;	/* no cached block */
    buf->b_ml.ml_local.ml_line_lnum = 0;	/* no cached line */
#ifdef FEAT_BYTEOFF
    buf->b_ml.ml_local.ml_chunksize = NULL;
#endif

    buf->b_ml.ml_has_remote = 0; /* start without remote */
    ...
    buf->b_ml.ml_local.ml_mfp = mfp;
#ifdef FEAT_CRYPT
    mfp->mf_buffer = buf;
#endif
    buf->b_ml.ml_local.ml_flags = ML_EMPTY;
    buf->b_ml.ml_local.ml_line_count = 1;
    ...
    ZERO_BL	*b0p;
{
	...
	else
	{
	    ...
	    mch_memmove(buf->b_ml.ml_local.ml_mfp->mf_seed, &b0p->b0_seed, MF_SEED_LEN);
	}
    }
}
...
ml_set_crypt_key(buf, old_key, old_cm)
    ...
    int		old_cm;
{
    memfile_T	*mfp = buf->b_ml.ml_local.ml_mfp;
    bhdr_T	*hp;
    int		page_count;
    int		idx;
    long	error;
	...
	error = 0;
	buf->b_ml.ml_local.ml_stack_top = 0;
	vim_free(buf->b_ml.ml_local.ml_stack);
	buf->b_ml.ml_local.ml_stack = NULL;
	buf->b_ml.ml_local.ml_stack_size = 0;	/* no stack yet */
	...
	for ( ; !got_int; line_breakcheck())
	{
		...
		if (pp->pb_id == PTR_ID)	/* it is a pointer block */
		{
			...
			ip = &(buf->b_ml.ml_local.ml_stack[top]);
			ip->ip_bnum = bnum;
			ip->ip_index = idx;
	    ...
	    if (buf->b_ml.ml_local.ml_stack_top == 0)	/* finished */
		break;

	    /* go one block up in the tree */
	    ip = &(buf->b_ml.ml_local.ml_stack[--(buf->b_ml.ml_local.ml_stack_top)]);
	    bnum = ip->ip_bnum;
...
ml_close(buf, del_file)
    ...
    int		del_file;
{
    if (buf->b_ml.ml_local.ml_mfp == NULL)		/* not open */
	return;
    mf_close(buf->b_ml.ml_local.ml_mfp, del_file);	/* close the .swp file */
    if (buf->b_ml.ml_local.ml_line_lnum != 0 && (buf->b_ml.ml_local.ml_flags & ML_LINE_DIRTY))
	vim_free(buf->b_ml.ml_local.ml_line_ptr);
    vim_free(buf->b_ml.ml_local.ml_stack);
#ifdef FEAT_BYTEOFF
    vim_free(buf->b_ml.ml_local.ml_chunksize);
    buf->b_ml.ml_local.ml_chunksize = NULL;
#endif
    buf->b_ml.ml_local.ml_mfp = NULL;
    buf->b_ml.ml_local.ml_line_count = 0;    /* no lines in buffer */
...
set_b0_dir_flag(b0p, buf)
    ...
    buf_T	*buf;
{
    if (same_directory(buf->b_ml.ml_local.ml_mfp->mf_fname, buf->b_ffname))
...
ml_recover()
{
    ...
    recoverymode = TRUE;
    called_from_main = (curbuf->b_ml.ml_local.ml_mfp == NULL);
    attr = hl_attr(HLF_E);
    ...
    buf->b_ml.ml_local.ml_stack_top = 0;		/* nothing in the stack */
    buf->b_ml.ml_local.ml_line_lnum = 0;		/* no cached line */
    buf->b_ml.ml_local.ml_locked = NULL;		/* no locked block */
    buf->b_ml.ml_local.ml_flags = 0;
#ifdef FEAT_CRYPT
     ...
     */
    while (!(curbuf->b_ml.ml_local.ml_flags & ML_EMPTY))
     ...
     */
    if (orig_file_status != OK || curbuf->b_ml.ml_local.ml_line_count != lnum * 2 + 1)
    {
	/* Recovering an empty file results in two lines and the first line is
	 * empty.  Don't set the modified flag then. */
	if (!(curbuf->b_ml.ml_local.ml_line_count == 2 && *ml_get(1) == NUL))
	{
	    changed_int();
     ...
     */
    while (curbuf->b_ml.ml_local.ml_line_count > lnum
				       && !(curbuf->b_ml.ml_local.ml_flags & ML_EMPTY))
	ml_delete(curbuf->b_ml.ml_local.ml_line_count, FALSE);
...
# endif
	 ...
	 */
	if (curbuf->b_ml.ml_local.ml_mfp != NULL
			       && (p = curbuf->b_ml.ml_local.ml_mfp->mf_fname) != NULL)
	{
    ...
    int	    check_char;
{
    ...
    for (buf = firstbuf; buf != NULL; buf = buf->b_next)
    {
	if (buf->b_ml.ml_local.ml_mfp == NULL || buf->b_ml.ml_local.ml_mfp->mf_fname == NULL)
	    continue;			    /* no file */

	ml_flush_line(buf);		    /* flush buffered line */
					    /* flush locked block */
	(void)ml_find_line(buf, (linenr_T)0, ML_FLUSH);
	if (bufIsChanged(buf) && check_file && mf_need_trans(buf->b_ml.ml_local.ml_mfp)
						     && buf->b_ffname != NULL)
	{
	    /*
	...
	if (buf->b_ml.ml_local.ml_mfp->mf_dirty)
	{
	    (void)mf_sync(buf->b_ml.ml_local.ml_mfp, (check_char ? MFS_STOP : 0)
    ...
    int		message;
{
    ...
    {
	lnum = 1;
	while (mf_need_trans(mfp) && lnum <= buf->b_ml.ml_local.ml_line_count)
	{
	    ...
	    CHECK(buf->b_ml.ml_local.ml_locked_low != lnum, "low != lnum");
	    lnum = buf->b_ml.ml_local.ml_locked_high + 1;
	}
	(void)ml_find_line(buf, (linenr_T)0, ML_FLUSH);	/* flush locked block */
	/* sync the updated pointer blocks */
	if (mf_sync(mfp, MFS_ALL | MFS_FLUSH) == FAIL)
	    status = FAIL;
	buf->b_ml.ml_local.ml_stack_top = 0;	    /* stack is invalid now */
    }
theend:
    ...
    int		will_change;		/* line will be changed */
{
    ...
    if (lnum > buf->b_ml.ml_local.ml_line_count)	/* invalid line number */
    {
	if (recursive == 0)
	{
...
errorret:
    ...
    if (buf->b_ml.ml_local.ml_mfp == NULL)	/* there are no lines */
	return (char_u *)"";
     ...
     * blocks.
     */
    if (buf->b_ml.ml_local.ml_line_lnum != lnum || mf_dont_release)
    {
	ml_flush_line(buf);
	...
	ptr = (char_u *)dp + ((dp->db_index[lnum - buf->b_ml.ml_local.ml_locked_low]) & DB_INDEX_MASK);
	buf->b_ml.ml_local.ml_line_ptr = ptr;
	buf->b_ml.ml_local.ml_line_lnum = lnum;
	buf->b_ml.ml_local.ml_flags &= ~ML_LINE_DIRTY;
    }
    if (will_change)
	buf->b_ml.ml_local.ml_flags |= (ML_LOCKED_DIRTY | ML_LOCKED_POS);

    return buf->b_ml.ml_local.ml_line_ptr;
}
...
ml_line_alloced()
{
    return (curbuf->b_ml.ml_local.ml_flags & ML_LINE_DIRTY);
}
    ...
    int		newfile;	/* flag, see above */
{
    ...
    if (curbuf->b_ml.ml_local.ml_mfp == NULL && open_buffer(FALSE, NULL, 0) == FAIL)
	return FAIL;

    if (curbuf->b_ml.ml_local.ml_line_lnum != 0)
	ml_flush_line(curbuf);
    ret = ml_append_int(curbuf, lnum, line, len, newfile, FALSE);
    ...
    int		newfile;	/* flag, see above */
{
    ...
    if (buf->b_ml.ml_local.ml_mfp == NULL)
	return FAIL;

    if (buf->b_ml.ml_local.ml_line_lnum != 0)
	ml_flush_line(buf);
    ret = ml_append_int(buf, lnum, line, len, newfile, FALSE);
    ...
    int		mark;		/* mark the new line */
{
    ...
    infoptr_T	*ip;
					...
					/* lnum out of range */
    if (lnum > buf->b_ml.ml_local.ml_line_count || buf->b_ml.ml_local.ml_mfp == NULL)
	return FAIL;
 ...
 */
	...
	db_idx = lnum - buf->b_ml.ml_local.ml_locked_low;
		/* get line count before the insertion */
    line_count = buf->b_ml.ml_local.ml_locked_high - buf->b_ml.ml_local.ml_locked_low;
 ...
 */
    if ((int)dp->db_free < space_needed && db_idx == line_count - 1
					    && lnum < buf->b_ml.ml_local.ml_line_count)
    {
	/*
	 ...
	 */
	--(buf->b_ml.ml_local.ml_locked_lineadd);
	--(buf->b_ml.ml_local.ml_locked_high);
	...
	db_idx = -1;		    /* careful, it is negative! */
		    /* get line count before the insertion */
	line_count = buf->b_ml.ml_local.ml_locked_high - buf->b_ml.ml_local.ml_locked_low;
	CHECK(buf->b_ml.ml_local.ml_locked_low != lnum + 1, "locked_low != lnum + 1");

	dp = (DATA_BL *)(hp->bh_data);
    }

    ++buf->b_ml.ml_local.ml_line_count;
 ...
 */
	 ...
	 */
	buf->b_ml.ml_local.ml_flags |= ML_LOCKED_DIRTY;
	if (!newfile)
	    buf->b_ml.ml_local.ml_flags |= ML_LOCKED_POS;
    }
 ...
 */
	 ...
	 */
	lineadd = buf->b_ml.ml_local.ml_locked_lineadd;
	buf->b_ml.ml_local.ml_locked_lineadd = 0;
	ml_find_line(buf, (linenr_T)0, ML_FLUSH);   /* flush data block */

	/*
	 * update pointer blocks for the new data block
	 */
	for (stack_idx = buf->b_ml.ml_local.ml_stack_top - 1; stack_idx >= 0;
								  --stack_idx)
	{
	    ip = &(buf->b_ml.ml_local.ml_stack[stack_idx]);
	    pb_idx = ip->ip_index;
		...
		mf_put(mfp, hp, TRUE, FALSE);
		buf->b_ml.ml_local.ml_stack_top = stack_idx + 1;	    /* truncate stack */

		if (lineadd)
		{
		    --(buf->b_ml.ml_local.ml_stack_top);
		    /* fix line count for rest of blocks in the stack */
		    ml_lineadd(buf, lineadd);
							/* fix stack itself */
		    buf->b_ml.ml_local.ml_stack[buf->b_ml.ml_local.ml_stack_top].ip_high +=
								      lineadd;
		    ++(buf->b_ml.ml_local.ml_stack_top);
		}
		...
		for (;;)	/* do this twice when splitting block 1 */
		{
		    ...
		    pp->pb_pointer[0].pe_line_count = buf->b_ml.ml_local.ml_line_count;
		    pp->pb_pointer[0].pe_old_lnum = 1;
		    pp->pb_pointer[0].pe_page_count = 1;
	...
	if (stack_idx < 0)
	{
	    ...
	    buf->b_ml.ml_local.ml_stack_top = 0;	/* invalidate stack */
	}
    }
...
#endif
    if (curbuf->b_ml.ml_local.ml_line_lnum != lnum)	    /* other line buffered */
	ml_flush_line(curbuf);			    /* flush it */
    else if (curbuf->b_ml.ml_local.ml_flags & ML_LINE_DIRTY) /* same line allocated */
	vim_free(curbuf->b_ml.ml_local.ml_line_ptr);	    /* free it */
    curbuf->b_ml.ml_local.ml_line_ptr = line;
    curbuf->b_ml.ml_local.ml_line_lnum = lnum;
    curbuf->b_ml.ml_local.ml_flags = (curbuf->b_ml.ml_local.ml_flags | ML_LINE_DIRTY) & ~ML_EMPTY;
    ...
    int		message;
{
    ...
    ret = ml_delete_int(curbuf, lnum, message);

    // FIXME: think about FAIL-case
    if(curbuf->b_ml.ml_has_remote)
        rl_delete_remote(curbuf, lnum);
    ...
    int		message;
{
    ...
    if (lnum < 1 || lnum > buf->b_ml.ml_local.ml_line_count)
	return FAIL;
 ...
 */
    if (buf->b_ml.ml_local.ml_line_count == 1)	    /* file becomes empty */
    {
	if (message
#ifdef FEAT_NETBEANS_INTG
 ...
 */
    ...
    count = (long)(buf->b_ml.ml_local.ml_locked_high)
					- (long)(buf->b_ml.ml_local.ml_locked_low) + 2;
    idx = lnum - buf->b_ml.ml_local.ml_locked_low;
 ...
 */
	...
	buf->b_ml.ml_local.ml_locked = NULL;

	for (stack_idx = buf->b_ml.ml_local.ml_stack_top - 1; stack_idx >= 0;
								  --stack_idx)
	{
	    buf->b_ml.ml_local.ml_stack_top = 0;	    /* stack is invalid when failing */
	    ip = &(buf->b_ml.ml_local.ml_stack[stack_idx]);
	    idx = ip->ip_index;
	    if ((hp = mf_get(mfp, ip->ip_bnum, 1)) == NULL)
		return FAIL;
		...
		buf->b_ml.ml_local.ml_stack_top = stack_idx;	/* truncate stack */
		/* fix line count for rest of blocks in the stack */
		if (buf->b_ml.ml_local.ml_locked_lineadd != 0)
		{
		    ml_lineadd(buf, buf->b_ml.ml_local.ml_locked_lineadd);
		    buf->b_ml.ml_local.ml_stack[buf->b_ml.ml_local.ml_stack_top].ip_high +=
						  buf->b_ml.ml_local.ml_locked_lineadd;
		}
		++(buf->b_ml.ml_local.ml_stack_top);
    ...
    linenr_T lnum;
{
    ...
    DATA_BL *dp;
				    /* invalid line number */
    if (lnum < 1 || lnum > curbuf->b_ml.ml_local.ml_line_count
					       || curbuf->b_ml.ml_local.ml_mfp == NULL)
    ...
    dp = (DATA_BL *)(hp->bh_data);
    dp->db_index[lnum - curbuf->b_ml.ml_local.ml_locked_low] |= DB_MARKED;
    curbuf->b_ml.ml_local.ml_flags |= ML_LOCKED_DIRTY;
}
...
ml_firstmarked()
{
     ...
     */
    for (lnum = lowest_marked; lnum <= curbuf->b_ml.ml_local.ml_line_count; )
    {
	/*
	 * Find the data block containing the line.
	 * This also fills the stack with the blocks from the root to the data
	...
	for (i = lnum - curbuf->b_ml.ml_local.ml_locked_low;
			    lnum <= curbuf->b_ml.ml_local.ml_locked_high; ++i, ++lnum)
...
ml_clearmarked()
{
    ...
    if (curbuf->b_ml.ml_local.ml_mfp == NULL)	    /* nothing to do */
	return;
    ...
    buf_T	*buf;
{
    ...
    if (buf->b_ml.ml_local.ml_line_lnum == 0 || buf->b_ml.ml_local.ml_mfp == NULL)
	return;		/* nothing to do */

    if (buf->b_ml.ml_local.ml_flags & ML_LINE_DIRTY)
    {
	...
	lnum = buf->b_ml.ml_local.ml_line_lnum;
	new_line = buf->b_ml.ml_local.ml_line_ptr;
	...
	else
	{
		...
		count = buf->b_ml.ml_local.ml_locked_high - buf->b_ml.ml_local.ml_locked_low + 1;
    ...
    buf->b_ml.ml_local.ml_line_lnum = 0;
}
    ...
    int		action;
{
     ...
     */
    if (buf->b_ml.ml_local.ml_locked)
    {
	if (ML_SIMPLE(action)
		&& buf->b_ml.ml_local.ml_locked_low <= lnum
		&& buf->b_ml.ml_local.ml_locked_high >= lnum
		&& !mf_dont_release)
	{
	    ...
	    return (buf->b_ml.ml_local.ml_locked);
	}

	mf_put(mfp, buf->b_ml.ml_local.ml_locked, buf->b_ml.ml_local.ml_flags & ML_LOCKED_DIRTY,
					    buf->b_ml.ml_local.ml_flags & ML_LOCKED_POS);
	buf->b_ml.ml_local.ml_locked = NULL;
    ...
    page_count = 1;
    low = 1;
    high = buf->b_ml.ml_local.ml_line_count;

    if (action == ML_FIND)	/* first try stack entries */
    {
	for (top = buf->b_ml.ml_local.ml_stack_top - 1; top >= 0; --top)
	{
	    ip = &(buf->b_ml.ml_local.ml_stack[top]);
		...
		low = ip->ip_low;
		high = ip->ip_high;
		buf->b_ml.ml_local.ml_stack_top = top;	/* truncate stack at prev entry */
		break;
	    }
	}
	if (top < 0)
	    buf->b_ml.ml_local.ml_stack_top = 0;		/* not found, start at the root */
    }
    else	/* ML_DELETE or ML_INSERT */
	buf->b_ml.ml_local.ml_stack_top = 0;	/* start at the root */
 ...
 */
	...
	{
	    buf->b_ml.ml_local.ml_locked = hp;
	    buf->b_ml.ml_local.ml_locked_low = low;
	    buf->b_ml.ml_local.ml_locked_high = high;
	    buf->b_ml.ml_local.ml_locked_lineadd = 0;
	    buf->b_ml.ml_local.ml_flags &= ~(ML_LOCKED_DIRTY | ML_LOCKED_POS);
	    return hp;
	}
	...
	{
	    if (lnum > buf->b_ml.ml_local.ml_line_count)
    ...
    buf_T	*buf;
{
    ...
    top = buf->b_ml.ml_local.ml_stack_top;

    /* may have to increase the stack size */
    if (top == buf->b_ml.ml_local.ml_stack_size)
    {
	CHECK(top > 0, _("Stack size increases")); /* more than 5 levels??? */

	newstack = (infoptr_T *)alloc((unsigned)sizeof(infoptr_T) *
					(buf->b_ml.ml_local.ml_stack_size + STACK_INCR));
	if (newstack == NULL)
	    return -1;
	mch_memmove(newstack, buf->b_ml.ml_local.ml_stack,
					     (size_t)top * sizeof(infoptr_T));
	vim_free(buf->b_ml.ml_local.ml_stack);
	buf->b_ml.ml_local.ml_stack = newstack;
	buf->b_ml.ml_local.ml_stack_size += STACK_INCR;
    }
    ...
    int		count;
{
    ...
    for (idx = buf->b_ml.ml_local.ml_stack_top - 1; idx >= 0; --idx)
    {
	ip = &(buf->b_ml.ml_local.ml_stack[idx]);
	if ((hp = mf_get(mfp, ip->ip_bnum, 1)) == NULL)
	    break;
	pp = (PTR_BL *)(hp->bh_data);	/* must be pointer block */
	if (pp->pb_id != PTR_ID)
    ...
    buf_T	*buf;
{
    ...
    if (!buf->b_ml.ml_local.ml_mfp)
	return;
    for (hp = buf->b_ml.ml_local.ml_mfp->mf_used_last; hp != NULL; hp = hp->bh_prev)
    {
	if (hp->bh_bnum == 0)
	{
	    b0p = (ZERO_BL *)(hp->bh_data);
	    ...
	    mf_sync(buf->b_ml.ml_local.ml_mfp, MFS_ZERO);
	    break;
	}
    }
}
    ...
    int		updtype;
{
    ...
    if (buf->b_ml.ml_local.ml_usedchunks == -1 || len == 0)
	return;
    if (buf->b_ml.ml_local.ml_chunksize == NULL)
    {
	buf->b_ml.ml_local.ml_chunksize = (chunksize_T *)
				  alloc((unsigned)sizeof(chunksize_T) * 100);
	if (buf->b_ml.ml_local.ml_chunksize == NULL)
	{
	    buf->b_ml.ml_local.ml_usedchunks = -1;
	    return;
	}
	buf->b_ml.ml_local.ml_numchunks = 100;
	buf->b_ml.ml_local.ml_usedchunks = 1;
	buf->b_ml.ml_local.ml_chunksize[0].mlcs_numlines = 1;
	buf->b_ml.ml_local.ml_chunksize[0].mlcs_totalsize = 1;
    }

    if (updtype == ML_CHNK_UPDLINE && buf->b_ml.ml_local.ml_line_count == 1)
    {
	/*
	...
	buf->b_ml.ml_local.ml_usedchunks = 1;
	buf->b_ml.ml_local.ml_chunksize[0].mlcs_numlines = 1;
	buf->b_ml.ml_local.ml_chunksize[0].mlcs_totalsize =
				  (long)STRLEN(buf->b_ml.ml_local.ml_line_ptr) + 1;
	return;
    }
	...
	for (curline = 1, curix = 0;
	     curix < buf->b_ml.ml_local.ml_usedchunks - 1
	     && line >= curline + buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines;
	     curix++)
	{
	    curline += buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines;
	}
    }
    else if (line >= curline + buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines
		 && curix < buf->b_ml.ml_local.ml_usedchunks - 1)
    {
    ...
    curchnk = buf->b_ml.ml_local.ml_chunksize + curix;
	...
	if (buf->b_ml.ml_local.ml_usedchunks + 1 >= buf->b_ml.ml_local.ml_numchunks)
	{
	    buf->b_ml.ml_local.ml_numchunks = buf->b_ml.ml_local.ml_numchunks * 3 / 2;
	    buf->b_ml.ml_local.ml_chunksize = (chunksize_T *)
		vim_realloc(buf->b_ml.ml_local.ml_chunksize,
			    sizeof(chunksize_T) * buf->b_ml.ml_local.ml_numchunks);
	    if (buf->b_ml.ml_local.ml_chunksize == NULL)
	    {
	...
	if (buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines >= MLCS_MAXL)
	{
	    int	    count;	    /* number of entries in block */
	    int	    idx;
	    int	    text_end;
	    ...
	    mch_memmove(buf->b_ml.ml_local.ml_chunksize + curix + 1,
			buf->b_ml.ml_local.ml_chunksize + curix,
			(buf->b_ml.ml_local.ml_usedchunks - curix) *
			sizeof(chunksize_T));
	    /* Compute length of first half of lines in the split chunk */
	    size = 0;
	    linecnt = 0;
	    while (curline < buf->b_ml.ml_local.ml_line_count
			&& linecnt < MLCS_MINL)
	    {
		...
		dp = (DATA_BL *)(hp->bh_data);
		count = (long)(buf->b_ml.ml_local.ml_locked_high) -
			(long)(buf->b_ml.ml_local.ml_locked_low) + 1;
		idx = curline - buf->b_ml.ml_local.ml_locked_low;
		curline = buf->b_ml.ml_local.ml_locked_high + 1;
		if (idx == 0)/* first line in block, text at the end */
	    ...
	    buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines = linecnt;
	    buf->b_ml.ml_local.ml_chunksize[curix + 1].mlcs_numlines -= linecnt;
	    buf->b_ml.ml_local.ml_chunksize[curix].mlcs_totalsize = size;
	    buf->b_ml.ml_local.ml_chunksize[curix + 1].mlcs_totalsize -= size;
	    buf->b_ml.ml_local.ml_usedchunks++;
	    ml_upd_lastbuf = NULL;   /* Force recalc of curix & curline */
	    return;
	}
	else if (buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines >= MLCS_MINL
		     && curix == buf->b_ml.ml_local.ml_usedchunks - 1
		     && buf->b_ml.ml_local.ml_line_count - line <= 1)
	{
	    /*
	     * We are in the last chunk and it is cheap to crate a new one
	     * after this. Do it now to avoid the loop above later on
	     */
	    curchnk = buf->b_ml.ml_local.ml_chunksize + curix + 1;
	    buf->b_ml.ml_local.ml_usedchunks++;
	    if (line == buf->b_ml.ml_local.ml_line_count)
	    {
		 ...
		 */
		hp = ml_find_line(buf, buf->b_ml.ml_local.ml_line_count, ML_FIND);
		if (hp == NULL)
		{
		    buf->b_ml.ml_local.ml_usedchunks = -1;
		    return;
	...
	curchnk->mlcs_numlines--;
	ml_upd_lastbuf = NULL;   /* Force recalc of curix & curline */
	if (curix < (buf->b_ml.ml_local.ml_usedchunks - 1)
	...
	{
	    buf->b_ml.ml_local.ml_usedchunks--;
	    mch_memmove(buf->b_ml.ml_local.ml_chunksize, buf->b_ml.ml_local.ml_chunksize + 1,
			buf->b_ml.ml_local.ml_usedchunks * sizeof(chunksize_T));
	    return;
	}
	...
	buf->b_ml.ml_local.ml_usedchunks--;
	if (curix < buf->b_ml.ml_local.ml_usedchunks)
	{
	    mch_memmove(buf->b_ml.ml_local.ml_chunksize + curix,
			buf->b_ml.ml_local.ml_chunksize + curix + 1,
			(buf->b_ml.ml_local.ml_usedchunks - curix) *
			sizeof(chunksize_T));
	}
	return;
    ...
    long	*offp;
{
    ...
    if (buf->b_ml.ml_local.ml_usedchunks == -1
	    || buf->b_ml.ml_local.ml_chunksize == NULL
	    || lnum < 0)
	return -1;
    ...
    curline = 1;
    curix = size = 0;
    while (curix < buf->b_ml.ml_local.ml_usedchunks - 1
	    && ((lnum != 0
	     && lnum >= curline + buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines)
		|| (offset != 0
	       && offset > size + buf->b_ml.ml_local.ml_chunksize[curix].mlcs_totalsize
		      + ffdos * buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines)))
    {
	curline += buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines;
	size += buf->b_ml.ml_local.ml_chunksize[curix].mlcs_totalsize;
	if (offset && ffdos)
	    size += buf->b_ml.ml_local.ml_chunksize[curix].mlcs_numlines;
	curix++;
    }

    while ((lnum != 0 && curline < lnum) || (offset != 0 && size < offset))
    {
	if (curline > buf->b_ml.ml_local.ml_line_count
		|| (hp = ml_find_line(buf, curline, ML_FIND)) == NULL)
	    return -1;
	dp = (DATA_BL *)(hp->bh_data);
	count = (long)(buf->b_ml.ml_local.ml_locked_high) -
		(long)(buf->b_ml.ml_local.ml_locked_low) + 1;
	start_idx = idx = curline - buf->b_ml.ml_local.ml_locked_low;
	if (idx == 0)/* first line in block, text at the end */
	    text_end = dp->db_txt_end;
	else
	    text_end = ((dp->db_index[idx - 1]) & DB_INDEX_MASK);
    ...
    long	cnt;
{
    ...
    {
	curwin->w_cursor.lnum = curbuf->b_ml.ml_local.ml_line_count;
...
/*
 * Has a memfile been allocated for this memline?
 */
    int
ml_has_memfile(buf)
    buf_T *buf;
{
    return buf->b_ml.ml_local.ml_mfp != NULL;
}
...
ml_line_count(buf)
    buf_T *buf;
{
    return buf->b_ml.ml_local.ml_line_count;
}

/*
 * remove the memfile from this memlines
 */
    void
ml_dissociate_memfile(buf)
    buf_T *buf;
{
    buf->b_ml.ml_local.ml_line_count = 1;
    buf->b_ml.ml_local.ml_mfp = NULL;
    buf->b_ml.ml_local.ml_flags = ML_EMPTY;		/* empty buffer */
}

/*
 * return memline flags
 */
    int
ml_flags(buf)
    buf_T *buf;
{
    return buf->b_ml.ml_local.ml_flags;
}
 ...
 */
    int
ml_set_flags(buf, flags)
    buf_T *buf;
    int   flags;
{
    buf->b_ml.ml_local.ml_flags = flags;
}

/*
 * current swapfile name
 */
    char *
ml_swapfile_name(buf)
    buf_T *buf;
{
    return buf->b_ml.ml_local.ml_mfp->mf_fname;
}

/*
 * mf_fullname - memline version
 */
    void
ml_mf_fullname(buf)
    buf_T *buf;
{
    mf_fullname(buf->b_ml.ml_local.ml_mfp);
}
...
/*
 * does this memline have a remote master
 */
    int
ml_has_remote(buf)
    buf_T *buf;
{
    return buf->b_ml.ml_has_remote;
}
