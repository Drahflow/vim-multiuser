...
# endif
   ...
   {"printmbcharset", "pmbcs",  P_STRING|P_VI_DEF,
    ...
    {"remap",	    NULL,   P_BOOL|P_VI_DEF,
			    ...
			    {(char_u *)TRUE, (char_u *)0L} SCRIPTID_INIT},
    {"remote",	    "rem",  P_STRING|P_ALLOCED|P_VI_DEF|P_NOGLOB|P_NODEFAULT,
			    (char_u *)&p_remote, PV_REM,
			    {(char_u *)"", (char_u *)0L}
			    SCRIPTID_INIT},
...
check_buf_options(buf)
    buf_T	*buf;
{
    check_string_option(&buf->b_p_rem);
#if defined(FEAT_QUICKFIX)
...
#endif
	if (varp == &(curbuf->b_p_rem))
	{
            rl_connect_remote(curbuf, curbuf->b_p_rem);
	}
    }
	...
	*(int *)get_varp_scope(&(options[opt_idx]), OPT_GLOBAL) = value;
	...
	if (curbuf->b_p_udf || p_udf)
	{
		...
		if ((curbuf == save_curbuf
			...
			&& !curbufIsChanged() && ml_has_memfile(curbuf))
		{
		    u_compute_hash(hash);
...
#endif
	case PV_REM:	return (char_u *)&(curbuf->b_p_rem);
