...
win_found:
	...
	if (qf_ptr->qf_pattern == NULL)
	{
	    ...
	    if (i > 0)
	    {
		if (i > ml_line_count(curbuf))
		    i = ml_line_count(curbuf);
    ...
    int		old_qf_index;	/* previous qf_index or zero */
{
    ...
    win = qf_find_win(qi);
    if (win != NULL
	    && qf_index <= ml_line_count(win->w_buffer)
	    && old_qf_index != qf_index)
    {
    ...
    qf_info_T	*qi;
{
    ...
    while ((ml_flags(curbuf) & ML_EMPTY) == 0)
    ...
    exarg_T	*eap;
{
	...
	if (buf == NULL || !ml_has_memfile(buf))
	{
	...
	else
	{
	    ...
	    found_match = FALSE;
	    for (lnum = 1; lnum <= ml_line_count(buf) && tomatch > 0;
								       ++lnum)
	    {
		col = 0;
    ...
    exarg_T   *eap;
{
	...
	{
	    eap->line1 = 1;
	    eap->line2 = ml_line_count(buf);
	}
	if (eap->line1 < 1 || eap->line1 > ml_line_count(buf)
		|| eap->line2 < 1 || eap->line2 > ml_line_count(buf))
	    EMSG(_(e_invrange));
	else
	{
