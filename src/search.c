    ...
    proftime_T	*tm UNUSED;	/* timeout limit or NULL */
{
    ...
    /* Watch out for the "col" being MAXCOL - 2, used in a closed fold. */
		    ...
		    && pos->lnum >= 1 && pos->lnum <= ml_line_count(buf)
						     && pos->col < MAXCOL - 2)
    {
	...
	{
	    for ( ; lnum > 0 && lnum <= ml_line_count(buf);
		...
		if (nmatched > 0)
		{
		    ...
		    if (lnum + matchpos.lnum > ml_line_count(buf))
			ptr = (char_u *)"";
		    else
    ...
    proftime_T	    *tm;	/* timeout limit or NULL */
{
	...
	if (!spats[0].off.line && spats[0].off.off && pos.col < MAXCOL - 2)
	{
		...
		{
		    pos.lnum = ml_line_count(curbuf) + 1;
		    pos.col = 0;
		}
	    }
	}
	...
	if (!(options & SEARCH_NOOF) || (pat != NULL && *pat == ';'))
	{
		...
		if (c < 1)
		    pos.lnum = 1;
		else if (c > ml_line_count(curbuf))
		    pos.lnum = ml_line_count(curbuf);
		else
		    pos.lnum = c;
		pos.col = 0;
    ...
    char_u	*pat;
{
	...
	else if (pos->lnum > ml_line_count(buf))
	{
	    if (p_ws)
	    {
		pos->lnum = 1;
    ...
    int		maxtravel;
{
	...
	if (hash_dir)
	{
		...
		if (hash_dir > 0)
		{
		    if (pos.lnum == ml_line_count(curbuf))
			break;
		}
		else if (pos.lnum == 1)
		    break;
	...
	else				/* forward search */
	{
	    ...
	    {
		if (pos.lnum == ml_line_count(curbuf)  /* end of file */
#ifdef FEAT_LISP
    ...
    int		both;
{
	...
	for (first = TRUE; ; first = FALSE)
	{
		...
		curr -= dir;
#endif
	    if ((curr += dir) < 1 || curr > ml_line_count(curbuf))
	    {
		if (count)
		    return FALSE;
		curr -= dir;
    ...
    if (curr == ml_line_count(curbuf) && what != '}')
    {
    ...
    int		eol;
{
	 ...
	 */
	last_line = (curwin->w_cursor.lnum == ml_line_count(curbuf));
	i = inc_cursor();
...
extend:
	...
	{
	    if (start_lnum ==
			   (dir == BACKWARD ? 1 : ml_line_count(curbuf)))
	    {
		retval = FAIL;
		break;
	    }
     ...
     */
    end_lnum = start_lnum;
    while (end_lnum <= ml_line_count(curbuf) && linewhite(end_lnum))
	++end_lnum;
    ...
    while (i--)
    {
	if (end_lnum == ml_line_count(curbuf))
	    return FAIL;
	...
	if (include || !do_white)
	{
	     ...
	     */
	    while (end_lnum < ml_line_count(curbuf)
    ...
    int		forward;	/* move forward or backwards */
{
	...
	else if (!i && !result)
	{
		...
		pos.lnum = ml_line_count(curwin->w_buffer);
		pos.col  = (colnr_T)STRLEN(
				ml_get(ml_line_count(curwin->w_buffer)));
	    }
	}
	p_ws = old_p_ws;
    }
