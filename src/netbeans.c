...
/* =====================================================================*/
	...
	else if (streq((char *)cmd, "insert"))
	{
	    ...
	    nbdebug(("    CHUNK[%d]: %d bytes at offset %d\n",
		    ml_line_count(buf->bufp), STRLEN(args), off));
	    */
		...
		int	buf_was_empty = (ml_flags(buf->bufp) & ML_EMPTY);
		size_t	len = 0;
		int	added = 0;
		int	oldFire = netbeansFireChanges;
		int	old_b_changed;
		...
		else
		{
			...
			lnum_start = ml_line_count(curbuf) + 1;
		}
...
get_buf_size(buf_T *bufp)
{
	...
	if (get_fileformat(bufp) == EOL_DOS)
	    ...
	    eol_size = 1;
	for (lnum = 1; lnum <= ml_line_count(bufp); ++lnum)
	{
...
print_read_msg(buf)
    nbbuf_T	*buf;
{
    int	    lnum = ml_line_count(buf->bufp);
    ...
    off_t	nchars;
{
	...
	msg_add_lines(c, ml_line_count(buf->bufp),
