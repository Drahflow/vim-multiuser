...
# endif
	...
	if (VIsual_active || redo_VIsual_busy)
	{
			...
			)
		{
		    ...
		    {
			if (oap->end.lnum < ml_line_count(curbuf))
			{
			    ++oap->end.lnum;
			    oap->end.col = 0;
#ifdef FEAT_VIRTUALEDIT
    ...
    oparg_T	*oap;
{
	...
	if (oap->end.lnum != oap->start.lnum)
	{
		...
		stuffcharReadbuff('.');
	    else if (oap->end.lnum == ml_line_count(curbuf))
		stuffcharReadbuff('$');
    ...
    long	leftcol_diff;
{
	...
	if (curwin != old_curwin && curwin->w_p_scb)
	{
...
#endif
		{
		    ...
		    if (topline > ml_line_count(curbuf))
			topline = ml_line_count(curbuf);
		    if (topline < 1)
			topline = 1;
...
dozet:
    ...
    {
	setpcmark();
	if (cap->count0 > ml_line_count(curbuf))
	    curwin->w_cursor.lnum = ml_line_count(curbuf);
	else
    ...
    case 'E':	if (foldmethodIsManual(curwin))
		{
		    ...
		    deleteFold((linenr_T)1, ml_line_count(curbuf),
								 TRUE, FALSE);
		else
    ...
    cmdarg_T  *cap;
{
	...
	if (cmd_result == FAIL)
		...
		&& (cap->oap->start.lnum > ml_line_count(curbuf)
		    || cap->oap->start.col >
    ...
    cmdarg_T  *cap;
{
	...
	if (cap->cmdchar == 'M')
	{
	    ...
	    for (n = 0; curwin->w_topline + n < ml_line_count(curbuf); ++n)
	    {
#ifdef FEAT_DIFF
    ...
    cmdarg_T	*cap;
{
	...
	if (!checkclearop(cap->oap))
	{
	    ...
	    int	    line_count = ml_line_count(curbuf);
	    pos_T   start, end;
...
# endif
		...
		{
		    /* adjust lines */
		    VIsual.lnum += ml_line_count(curbuf) - line_count;
		    curwin->w_cursor.lnum +=
				      ml_line_count(curbuf) - line_count;
		}
    ...
    cmdarg_T	*cap;
{
	...
	else
	{
	    ...
	    if (ml_line_count(curbuf) > 1000000)
		curwin->w_cursor.lnum = (ml_line_count(curbuf) + 99L)
							 / 100L * cap->count0;
	    else
		curwin->w_cursor.lnum = (ml_line_count(curbuf) *
						    cap->count0 + 99L) / 100L;
    ...
    cmdarg_T	*cap;
{
	...
	if (	   curbuf->b_visual.vi_start.lnum == 0
		|| curbuf->b_visual.vi_start.lnum > ml_line_count(curbuf)
    ...
    cmdarg_T	*cap;
{
    ...
    if (lnum < 1L)
	lnum = 1L;
    else if (lnum > ml_line_count(curbuf))
	lnum = ml_line_count(curbuf);
    curwin->w_cursor.lnum = lnum;
    beginline(BL_SOL | BL_FIX);
#ifdef FEAT_FOLDING
    ...
    cmdarg_T  *cap;
{
	...
	if (VIsual_active)
	{
	   ...
	   if (regname == 0 || regname == '"'
	    ...
	    nv_operator(cap);
	    do_pending_operator(cap, 0, FALSE);
	    empty = ml_flags(curbuf) & ML_EMPTY;
	...
	if (empty && *ml_get(ml_line_count(curbuf)) == NUL)
	{
	    ml_delete(ml_line_count(curbuf), TRUE);
