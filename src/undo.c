 ...
 * "top" may be 0 and bot may be ml_line_count(curbuf) + 1.
 * Careful: may trigger autocommands that reload the buffer.
    ...
    linenr_T top, bot;
{
    ...
    if (top > ml_line_count(curbuf)
	    || top >= bot
	    || bot > ml_line_count(curbuf) + 1)
    ...
    long	nlines;
{
    ...
    return (u_savecommon(lnum - 1, lnum + nlines,
		     nlines == ml_line_count(curbuf) ? 2 : lnum, FALSE));
}
    ...
    int		reload;
{
	...
	uhp->uh_flags = (curbuf->b_changed ? UH_CHANGED : 0) +
		       ((ml_flags(curbuf) & ML_EMPTY) ? UH_EMPTYBUF : 0);
	...
	if (size == 1)
	{
		...
		if ((curbuf->b_u_newhead->uh_getbot_entry != uep
				...
				!= (uep->ue_bot == 0
				    ? ml_line_count(curbuf) + 1
				    : uep->ue_bot))
			    : uep->ue_lcount != ml_line_count(curbuf))
			|| (uep->ue_size > 1
			    && top >= uep->ue_top
		...
		if (uep->ue_size == 1 && uep->ue_top == top)
		{
		    ...
		    if (newbot != 0)
			uep->ue_bot = newbot;
		    else if (bot > ml_line_count(curbuf))
			uep->ue_bot = 0;
		    else
		    {
			uep->ue_lcount = ml_line_count(curbuf);
    ...
    char_u	*hash;
{
    ...
    put_bytes(fp, (long_u)ml_line_count(buf), 4);
    ...
    int		undo;
{
    ...
    curbuf->b_op_start.lnum = ml_line_count(curbuf);
	...
	if (bot == 0)
	    bot = ml_line_count(curbuf) + 1;
	if (top > ml_line_count(curbuf) || top >= bot
				      || bot > ml_line_count(curbuf) + 1)
	{
#ifdef FEAT_AUTOCMD
	    unblock_autocmds();
	...
	ml_set_flags(curbuf, ml_flags(curbuf) | ML_EMPTY);
...
u_find_first_changed()
{
    ...
    for (lnum = 1; lnum < ml_line_count(curbuf)
    ...
    if (ml_line_count(curbuf) != uep->ue_size)
    {
...
u_getbot()
{
	 ...
	 */
	extra = ml_line_count(curbuf) - uep->ue_lcount;
	uep->ue_bot = uep->ue_top + uep->ue_size + 1 + extra;
	if (uep->ue_bot < 1 || uep->ue_bot > ml_line_count(curbuf))
	{
    ...
    linenr_T lnum;
{
	...
	return;
    if (lnum < 1 || lnum > ml_line_count(curbuf)) /* should never happen */
	return;
    u_clearline();
...
u_undoline()
{
    ...
    if (curbuf->b_u_line_ptr == NULL
			|| curbuf->b_u_line_lnum > ml_line_count(curbuf))
    {
	beep_flush();
	return;
    }
