 ...
 */
	...
	{
	    if (ea.line2 > ml_line_count(curbuf))
	    {
...
ex_swapname(eap)
    exarg_T	*eap UNUSED;
{
    if (!ml_has_memfile(curbuf) || ml_swapfile_name(curbuf) == NULL)
	MSG(_("No swap file"));
    else
	msg(ml_swapfile_name(curbuf));
}
    ...
    exarg_T	*eap UNUSED;
{
	...
	for (wp = firstwin; wp; wp = wp->w_next)
	{
	    ...
	    {
		y = ml_line_count(wp->w_buffer) - p_so;
		if (topline > y)
		    topline = y;
	    }
	}
    ...
    exarg_T	*eap;
{
     ...
     */
    if (n == MAXLNUM || n < 0 || n > ml_line_count(curbuf))
    {
	EMSG(_(e_invaddr));
	return;
    }
