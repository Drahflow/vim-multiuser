...
# endif
    ...
    if (gui.in_use)
    {
        // FIXME: put polling logic into GUI path as well
...
# endif
	...
	{
	    count = 0;
	    for (first = TRUE; curwin->w_topline < ml_line_count(curbuf); )
	    {
#ifdef FEAT_DIFF
...
#endif
	if (lnum == ml_line_count(win->w_buffer))
	{
	    retval = TRUE;
