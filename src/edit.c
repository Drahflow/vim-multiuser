...
# endif
		&& save_cursor.lnum <= ml_line_count(curbuf))
	{
	...
	orig_line_count = ml_line_count(curbuf);
	vr_lines_changed = 1;
    }
#endif
    else
    ...
    pos_T	*ini;
{
					...
					&& (!compl_started || found_all))
	{
		...
		if (ml_has_memfile(ins_buf))   /* loaded buffer */
		{
		    compl_started = TRUE;
		    first_match_pos.col = last_match_pos.col = 0;
		    first_match_pos.lnum = ml_line_count(ins_buf) + 1;
	...
	default:	/* normal ^P/^N and ^X^L */
		...
		if (ctrl_x_mode == CTRL_X_WHOLE_LINE)
		{
		    ...
		    {
			if (pos->lnum >= ml_line_count(ins_buf))
			    continue;
    ...
    int		nomove;			/* <c-\><c-o>, don't move cursor */
{
	...
	if (!nomove && did_ai && (esc || (vim_strchr(p_cpo, CPO_INDENT) == NULL
		...
		&& end_insert_pos->lnum <= ml_line_count(curbuf))
	{
	    pos_T	tpos = curwin->w_cursor;
    ...
    int		upd_topline;	    /* When TRUE: update topline */
{
	...
	if (lnum >= ml_line_count(curbuf)
		|| (lnum + n > ml_line_count(curbuf)
		    && vim_strchr(p_cpo, CPO_MINUS) != NULL))
        {
	    return FAIL;
        }

	if (lnum + n >= ml_line_count(curbuf))
