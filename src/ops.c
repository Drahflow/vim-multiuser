    ...
    oparg_T   *oap;
{
    ...
    char_u		*newp, *oldp;
    struct block_def	bd;
    linenr_T		old_lcount = ml_line_count(curbuf);
    int			did_yank = FALSE;
    int			orig_regname = oap->regname;

    if (ml_flags(curbuf) & ML_EMPTY)	    /* nothing to do */
	return OK;
...
# endif
	...
	if (oap->line_count == 1)	/* delete characters within one line */
	{
	    ...
	    if (oap->op_type == OP_DELETE
		    && oap->inclusive
		    && oap->end.lnum == ml_line_count(curbuf)
	...
	else				/* delete characters between lines */
	{
	    ...
	    delete_last_line = (oap->end.lnum == ml_line_count(curbuf));
    ...
    int		c;
{
    ...
    if ((ml_flags(curbuf) & ML_EMPTY ) || oap->empty)
    ...
    int		flags;
{
	...
	for (i = 0; i < y_size; ++i)
	{
	    ...
	    if (curwin->w_cursor.lnum > ml_line_count(curbuf))
	    {
		if (ml_append(ml_line_count(curbuf), (char_u *)"",
...
{
    long	old_line_count = ml_line_count(curbuf);
     ...
     */
    if (oap->end_adjusted && curwin->w_cursor.lnum < ml_line_count(curbuf))
	++curwin->w_cursor.lnum;
    beginline(BL_WHITE | BL_FIX);
    old_line_count = ml_line_count(curbuf) - old_line_count;
    ...
    int		avoid_fex;		/* don't use 'formatexpr' */
{
	 ...
	 */
	if (count == 1 || curwin->w_cursor.lnum == ml_line_count(curbuf))
	{
