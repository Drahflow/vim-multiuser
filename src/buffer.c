    ...
    int		flags;		    /* extra flags for readfile() */
{
	...
	if (retval == OK)
	{
	    line_count = ml_line_count(curbuf);
		...
		while (ml_line_count(curbuf) > line_count)
...
# endif
	 ...
	 */
	if (buf_valid(old_curbuf) && ml_has_memfile(old_curbuf))
	{
	    aco_save_T	aco;
    ...
    int		forceit;	/* TRUE for :...! */
{
	...
	if (action != DOBUF_WIPE && !ml_has_memfile(buf) && !buf->b_p_bl)
	    return FAIL;
...
# endif
	...
	if (buf == NULL)	/* No previous buffer, Try 2'nd approach */
	{
		...
		{
		    if (ml_has_memfile(buf))   /* found loaded buffer */
			break;
    ...
    buf_T	*buf;
{
    ...
    if (!ml_has_memfile(curbuf))	/* need to load the file */
    {
#ifdef FEAT_AUTOCMD
    ...
    int		flags;		/* BLN_ defines */
{
    ...
    if ((flags & BLN_CURBUF)
	    ...
	    && curbuf->b_nwindows <= 1
	    && (!ml_has_memfile(curbuf) || bufempty()))
    {
	buf = curbuf;
#ifdef FEAT_AUTOCMD
    ...
    int		message;	/* give message when buffer already exists */
{
	...
	{
	    if (ml_has_memfile(buf))	/* it's loaded, fail */
	    {
		if (message)
    ...
    int	dont_truncate;
{
    ...
    *p++ = '"';
	...
	n = (int)(((long)curwin->w_cursor.lnum) /
				   ((long)ml_line_count(curbuf) / 100L));
    else
	n = (int)(((long)curwin->w_cursor.lnum * 100L) /
					    (long)ml_line_count(curbuf));
    if (ml_flags(curbuf) & ML_EMPTY)
    {
...
#define TMPLEN 70
	...
	case STL_LINE:
	    num = (ml_flags(wp->w_buffer) & ML_EMPTY)
		  ? 0L : (long)(wp->w_cursor.lnum);
	    break;

	case STL_NUMLINES:
	    num = ml_line_count(wp->w_buffer);
	    break;
	...
	case STL_PERCENTAGE:
	    num = (int)(((long)wp->w_cursor.lnum * 100L) /
			(long)ml_line_count(wp->w_buffer));
	    break;
	...
	case STL_OFFSET:
	    ...
	    num = (ml_flags(wp->w_buffer) & ML_EMPTY) || l < 0 ?
...
#endif
    below = ml_line_count(wp->w_buffer) - wp->w_botline + 1;
    ...
    exarg_T	*eap;
{
	...
	if ((!all && ml_has_memfile(buf)) || !buf->b_p_bl)
	    continue;
    ...
    int		flags;
{
    ...
    for (lnum = 1; lnum <= ml_line_count(curbuf) && lnum <= nmlines; ++lnum)
	if (chk_modeline(lnum, flags) == FAIL)
	    nmlines = 0;

    for (lnum = ml_line_count(curbuf); lnum > 0 && lnum > nmlines
		       && lnum > ml_line_count(curbuf) - nmlines; --lnum)
	if (chk_modeline(lnum, flags) == FAIL)
	    nmlines = 0;
    ...
    buf_T	*buf;
{
	...
	if (ml_line_count(buf) == ml_line_count(curbuf))
	{
	    differ = FALSE;
	    for (lnum = 1; lnum <= ml_line_count(curbuf); ++lnum)
