...
# endif
	...
	if (swap_mode > 0 && ml_has_memfile(curbuf)
			  && ml_swapfile_name(curbuf) != NULL)
	    (void)mch_setperm(ml_swapfile_name(curbuf), (long)swap_mode);
#endif
    }
    ...
    wasempty = (ml_flags(curbuf) & ML_EMPTY);
...
# endif
			   )
			{
				...
				goto rewind_retry;
			    if (conv_error == 0)
				conv_error = ml_line_count(curbuf)
								- linecnt + 1;
			}
			/* Remember the first linenr with an illegal byte */
			else if (illegal_byte == 0)
			    illegal_byte = ml_line_count(curbuf) - linecnt + 1;
...
# endif
	...
	if (newfile && wasempty && !(ml_flags(curbuf) & ML_EMPTY))
	{
#ifdef FEAT_NETBEANS_INTG
	    netbeansFireChanges = 0;
#endif
	    ml_delete(ml_line_count(curbuf), FALSE);
#ifdef FEAT_NETBEANS_INTG
	    netbeansFireChanges = 1;
#endif
	    --linecnt;
	}
	linecnt = ml_line_count(curbuf) - linecnt;
	if (filesize == 0)
	    linecnt = 0;
    ...
    char_u	*endp;		/* end of more bytes read */
{
    ...
    lnum = ml_line_count(curbuf) - linecnt + 1;
    for (s = p; s < endp; ++s)
	if (*s == '\n')
	    ++lnum;
    return lnum;
...
#endif
					/* writing everything */
    int		    whole = (start == 1 && end == ml_line_count(buf));
#ifdef FEAT_AUTOCMD
    linenr_T	    old_line_count = ml_line_count(buf);
#endif
    int		    attr;
    int		    fileformat;
    int		    write_bin;
...
# endif
	...
	int		empty_memline = !ml_has_memfile(buf);
	...
	if (!buf_valid(buf))
	    buf = NULL;
	if (buf == NULL || (!ml_has_memfile(buf) && !empty_memline)
				       || did_cmd || nofile_err
#ifdef FEAT_EVAL
				       || aborting()
#endif
	 ...
	 */
	if (ml_line_count(buf) != old_line_count)
	{
	    if (whole)						/* write all */
		end = ml_line_count(buf);
	    else if (ml_line_count(buf) > old_line_count)	/* more lines */
		end += ml_line_count(buf) - old_line_count;
	    else						/* less lines */
	    {
		end -= old_line_count - ml_line_count(buf);
		if (end < start)
		{
		    --no_wait_return;
		    msg_scroll = msg_save;
...
# endif
    ...
    if (end > ml_line_count(buf))
	end = ml_line_count(buf);
    if (ml_flags(buf) & ML_EMPTY)
	start = end + 1;
...
restore_backup:
	...
	if (end == 0
		|| (lnum == end
			...
			|| (lnum == ml_line_count(buf) && !buf->b_p_eol))))
	{
    ...
    buf_T	*tobuf;
{
    ...
    for (lnum = 1; lnum <= ml_line_count(frombuf); ++lnum)
    {
    ...
    {
	curbuf = frombuf;
	for (lnum = ml_line_count(curbuf); lnum > 0; --lnum)
    ...
    int		orig_mode;
{
	...
	if (p_ur < 0 || ml_line_count(curbuf) <= p_ur)
	{
	    /* Save all the text, so that the reload can be undone.
	     * Sync first so that this is a separate undo-able action. */
	    u_sync(FALSE);
	    saved = u_savecommon(0, ml_line_count(curbuf) + 1, 0, TRUE);
	...
	if (saved == OK)
	{
		...
		if (savebuf != NULL && buf_valid(savebuf) && buf == curbuf)
		{
		    ...
		    while (!bufempty())
			if (ml_delete(ml_line_count(buf), FALSE) == FAIL)
			    break;
	 ...
	 * have been removed). */
	if (old_topline > ml_line_count(curbuf))
	    curwin->w_topline = ml_line_count(curbuf);
	else
...
# endif
	...
	check_cursor();
	if (curwin->w_topline > ml_line_count(curbuf))
	{
	    curwin->w_topline = ml_line_count(curbuf);
#ifdef FEAT_DIFF
	    curwin->w_topfill = 0;
