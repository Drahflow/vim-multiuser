...
// FIXME: lock all buffers ever touched during expression evaluation
    ...
    typval_T	*rettv;
{
    ...
    buf = find_buffer(&argvars[0]);
    rettv->vval.v_number = (buf != NULL && ml_has_memfile(buf));
}
    ...
    typval_T	*rettv;
{
    ...
    pos = curwin->w_cursor;
    lnum = get_tv_lnum(argvars);
    if (lnum >= 1 && lnum <= ml_line_count(curbuf))
    {
	curwin->w_cursor.lnum = lnum;
    ...
    typval_T	*rettv;
{
	...
	if (fp->col == MAXCOL)
	{
	    ...
	    if (fp->lnum <= ml_line_count(curbuf))
    ...
    typval_T	*rettv;
{
    ...
    if ((linenr_T)vimvars[VV_FOLDSTART].vv_nr > 0
	    && (linenr_T)vimvars[VV_FOLDEND].vv_nr <= ml_line_count(curbuf)
    ...
    typval_T	*rettv;
{
    ...
    if (buf == NULL || !ml_has_memfile(buf) || start < 0)
	return;

    if (!retlist)
    {
	if (start >= 1 && start <= ml_line_count(buf))
    ...
    typval_T	*rettv;
{
    ...
    lnum = get_tv_lnum(argvars);
    if (lnum < 1 || lnum > ml_line_count(curbuf) + 1)
	rettv->vval.v_number = -1;
    else
    ...
    typval_T	*rettv;
{
    ...
    {
	if (lnum < 0 || lnum > ml_line_count(curbuf))
	{
	    lnum = 0;
	    break;
	}
    ...
    typval_T	*rettv;
{
    ...
    listitem_T	*li = NULL;
    long	added = 0;
    linenr_T	lcount = ml_line_count(curbuf);
	...
	if (line == NULL || lnum < 1 || lnum > ml_line_count(curbuf) + 1)
	    break;
    ...
    typval_T	*rettv;
{
    ...
    if (!transerr && lnum >= 1 && lnum <= ml_line_count(curbuf)
    ...
    typval_T	*rettv;
{
    ...
    fp = var2fpos(&argvars[0], FALSE, &fnum);
    if (fp != NULL && fp->lnum <= ml_line_count(curbuf)
						    && fnum == curbuf->b_fnum)
    {
    ...
    int		*fnum;		/* set to fnum for '0, 'A, etc. */
{
	...
	if (error || pos.lnum <= 0 || pos.lnum > ml_line_count(curbuf))
