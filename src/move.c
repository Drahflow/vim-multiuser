    ...
    win_T	*wp;
{
    ...
    for ( ; lnum <= ml_line_count(wp->w_buffer); ++lnum)
    {
#ifdef FEAT_FOLDING
	last = lnum;
	folded = FALSE;
...
update_topline()
{
	...
	if (check_topline)
	{
				      ...
				      lnum < curwin->w_topline + p_so; ++lnum)
		{
		    ...
		    if (lnum >= ml_line_count(curbuf) || n >= halfheight)
			break;
	...
	if (curwin->w_botline <= ml_line_count(curbuf))
	{
...
# endif
    ...
    if (curwin->w_botline > ml_line_count(curbuf) + 1)
	curwin->w_botline = ml_line_count(curbuf) + 1;
    ...
    lineoff_T	*lp;
{
	...
	lp->fill = 0;
#endif
	if (lp->lnum > ml_line_count(curbuf))
	    lp->height = MAXCOL;
	else
#ifdef FEAT_FOLDING
    ...
    int		always;
{
	...
	if (hasFolding(top, &top, NULL))
	    ...
	    i = plines(top);
	used += i;
	if (extra + i <= off && bot < ml_line_count(curbuf))
	{
#ifdef FEAT_FOLDING
...
#ifdef FEAT_DIFF
	if (wp->w_botline <= ml_line_count(wp->w_buffer))
	{
    ...
    int		set_topbot;
{
...
#endif
			    p_so))
		    || boff.lnum + 1 > ml_line_count(curbuf))
	...
	if (boff.lnum < ml_line_count(curbuf))
	{
	    /* Add one line below */
	    botline_forw(&boff);
	    used += boff.height;
...
cursor_correct()
{
	...
	if (cln > botline && curwin->w_botline <= ml_line_count(curbuf))
	{
	    curwin->w_cursor.lnum = botline;
	    curwin->w_valid &=
			    ~(VALID_WROW|VALID_WCOL|VALID_CHEIGHT|VALID_CROW);
    ...
    long	count;
{
    ...
    if (ml_line_count(curbuf) == 1)    /* nothing to do */
    {
	beep_flush();
	return FAIL;
    }
	 ...
	 */
	if (dir == FORWARD
		? ((curwin->w_topline >= ml_line_count(curbuf) - p_so)
		    && curwin->w_botline > ml_line_count(curbuf))
		: (curwin->w_topline == 1
#ifdef FEAT_DIFF
		    && curwin->w_topfill ==
				    diff_check_fill(curwin, curwin->w_topline)
	...
	if (dir == FORWARD)
	{
	    ...
	    else if (curwin->w_botline > ml_line_count(curbuf))
	    {
		/* at end of file */
		curwin->w_topline = ml_line_count(curbuf);
#ifdef FEAT_DIFF
	...
	else	/* dir == BACKWARDS */
	{
		...
		if (curwin->w_cursor.lnum > ml_line_count(curbuf))
	    ...
	    if (loff.lnum >= ml_line_count(curbuf))
	    {
		loff.lnum = ml_line_count(curbuf);
#ifdef FEAT_DIFF
		loff.fill = 0;
	    }
	    else
	...
	if (curwin->w_topline <= old_topline
				  && old_topline < ml_line_count(curbuf))
	{
    ...
    linenr_T	Prenum;
{
	 ...
	 * scroll the text up
	 */
	while (n > 0 && curwin->w_botline <= ml_line_count(curbuf))
	{
#ifdef FEAT_DIFF
	    if (curwin->w_topfill > 0)
	    {
		...
		do
		{
		    ...
		    ++curwin->w_botline;
		    room -= i;
		} while (curwin->w_botline <= ml_line_count(curbuf));
	    }
	}
